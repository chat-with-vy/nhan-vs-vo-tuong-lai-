...
```
;> thôi a bận xíu lát nhắn<br>
;> này phần đã fix bug test oki rồi<br>
```
    private static void CreateFakeRegisters(List<Register> registers, List<Student> students, List<Subject> subjects)
    {
        DateTime currentTime = DateTime.Now;
    ...
    private static void AddStudent(List<Student> students)
    {
        ...
        Student student = new Student(fullName, major);
        students.Add(student);
        Console.WriteLine($"====> Mã SV {student.StudentId} đã được tạo thành công");
    }
    ...
    private static void RemoveStudent(List<Student> students, List<Register> registers)
    {
        ...
        if (Console.ReadLine().Trim().ToLower() == "y")
        {
            registers.RemoveAll(r => r.Student.StudentId == studentId);
            int removed = students.RemoveAll(s => s.StudentId == studentId);
            Console.WriteLine(removed > 0 ? "Xóa thành công" : "Xóa thất bại");
        }
        else
...
class FullName
{
    ...
    public FullName(string fullName)
    {
        ...
        MidName = string.Join(" ", data, 1, data.Length - 2);
    }
...
class Student
{
    ...
    public override string ToString()
    {
        return $"{StudentId,-10}{FullName,-20}{Major,-15}";
    }
}
class Subject
{
    private static int AUTO_ID = 11000;
    public int SubjectId { get; set; }
    public string SubjectName { get; set; }
    public int Credit { get; set; }
    public int NumOfLesson { get; set; }
    public Subject(string name, int credit, int numOfLesson)
    {
        SubjectId = AUTO_ID++;
        SubjectName = name;
        Credit = credit;
    ...
    public override string ToString()
    {
        return $"{SubjectId,-10}{SubjectName,-15}{Credit,-10}{NumOfLesson,-10}";
    }
}
class Register
{
    ...
    public override string ToString()
    {
        return $"{RegisterId,-10}{Student.StudentId,-10}{Student.FullName,-20}" +
            $"{Subject.SubjectId,-10}{Subject.SubjectName,-15}{RegisterTime,-15}";
    }
}
```
;> từ class như z nên mới suy ra viết logic như thế e nhó<br>
;> thấy e có vẻ quan tâm mấy này nên share cho e coi chứ gì đâu<br>
;> viết cho e coi nổ con mắt lun<br>
;> e ăn đc tré hem<br>
;> e chắc thích ăn ngọt hả
